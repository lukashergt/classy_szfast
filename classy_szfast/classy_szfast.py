...
from .cosmopower import cp_tt_nn, cp_te_nn, cp_ee_nn, cp_ee_nn, cp_pp_nn, cp_pknl_nn, cp_pkl_nn, cp_der_nn, cp_da_nn, cp_h_nn, cp_s8_nn, cp_pkl_fftlog_alphas_real_nn, cp_pkl_fftlog_alphas_imag_nn, cp_pkl_fftlog_alphas_nus, cosmopower_derived_params_idx_dict
...
class Class_szfast(object):
    def __init__(self,
        ...
        if cosmo_model_dict[params_settings['cosmo_model']] in ['LCDM', 'LCDM_Mnu-d3', 'LCDM_OmegaK']:
            self.cp_z_interp = self.linspace(0.,self.cp_z_interp_zmax,4001)
        else:
    ...
    def calculate_pkl(self,
        ...
        if self.use_Amod:
            ...
            for zp in z_arr:
                params_dict_pp = params_dict.copy()
                params_dict_pp['z_pk_save_nonclass'] = [zp]
                predicted_pk_spectrum_z.append(self.cp_pkl_nn[self.cosmo_model].ten_to_predictions_np(params_dict_pp)[0])
